        ...
        if (length == 0) {
            ...
            if (_endp != null && filled <= 0) {
                ...
                try {
                    ...
                    if (totalFilled < 0) totalFilled = 0;
                ...
                case STATE_START:
                    if (_ajpRequestPacket.parsedInt()) {
                        ...
                        if (_magic != Ajp13RequestHeaders.MAGIC) {
                            throw new IOException("Bad AJP13 rcv packet: " +
                                "0x" + Integer.toHexString(_magic) + " expected " +
                                "0x" + Integer.toHexString(Ajp13RequestHeaders.MAGIC) + " " + this);
                        }
                    ...
                    if (_ajpRequestPacket.parsedInt()) {
                        ...
                        if (packetLength > _bufferSize)
                            throw new IOException("AJP13 packet (" + packetLength +
                                "bytes) too large for buffer (" + _bufferSize +
                                " bytes)");
...
//                            break;
                        ...
                        default:
                            ...
                            Log.warn("AJP13 message type ({}) not supported/regonised as a " +
                                "container request", Integer.toString(packetType));
                ...
                case STATE_REQUEST_ATTR_VALUE:
                    if (_ajpRequestPacket.parsedString()) {
                        ...
                        switch (_ajpRequestPacket.getAttributeType()) {
                            ...
                            // XXX How does this plug into the web containers authentication?
                            ...
                            case Ajp13RequestHeaders.JVM_ROUTE_ATTR:
                                ...
                                //     Note used in org.mortbay.jetty.servlet.HashSessionIdManager
                            ...
                            case Ajp13RequestHeaders.SSL_CIPHER_ATTR:
                                // XXX Implement! Investigate SslSocketConnector.customize()
                                break;
                            ...
                            case Ajp13RequestHeaders.REQUEST_ATTR:
                                ...
                                // New Jk API?
                                // Check if experimental or can they assumed to be supported
                            case Ajp13RequestHeaders.SSL_KEYSIZE_ATTR:
                                ...
                                // Used to lock down jk requests with a secreate key.
                            case Ajp13RequestHeaders.SECRET_ATTR:
                                // XXX Investigate safest way to deal with this...
                                //     should this tie into shutdown packet?
                                break;

                            case Ajp13RequestHeaders.STORED_METHOD_ATTR:
                                // XXX Confirm this should really overide previously parsed method?
                            ...
                            default:
                                Log.warn("Unsupported Ajp13 Request Attribute {}:{}", new Byte(_ajpRequestPacket.getAttributeType()), _ajpRequestPacket.getString());
                                break;
                        }
