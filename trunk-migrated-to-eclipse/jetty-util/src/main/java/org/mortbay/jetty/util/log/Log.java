...
{    
        ...
        AccessController.doPrivileged(new PrivilegedAction<Boolean>() 
            {
                public Boolean run() 
                { 
                    __logClass = System.getProperty("org.mortbay.jetty.util.log.class","org.mortbay.jetty.util.log.Slf4jLog"); 
    ...
    public static void setLogToParent(String name)
    {
        ...
        if (loader.getParent()!=null)
        {
            try
            {
                Class<?> uberlog = loader.getParent().loadClass("org.mortbay.jetty.util.log.Log");
