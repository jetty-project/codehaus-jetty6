...
public class XCPSCachingServerAuthentication implements ServerAuthentication {
    ...
    private final CrossContextPsuedoSession<ServerAuthResult> xcps;
    ...
    public ServerAuthResult validateRequest(JettyMessageInfo messageInfo) throws ServerAuthException {
        ...
        ServerAuthResult serverAuthResult = xcps.fetch(request);
        if (serverAuthResult != null)
            return serverAuthResult;

        serverAuthResult = delegate.validateRequest(messageInfo);
        if (serverAuthResult != null)
            xcps.store(serverAuthResult, messageInfo.getResponseMessage());
